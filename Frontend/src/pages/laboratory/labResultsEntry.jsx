...
export default function PatientWiseReport() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans text-slate-800">
      ...
      {/* PRINT STYLES: Hides everything except the report content when printing */}
      <style>
        {`
          @media print {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {loading && !reportData ? (
           ...
           <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in duration-500">
            ...
            {/* LEFT: PATIENT INFO (Hidden on Print if desired, or keep it) */}
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div className="px-6 pb-6 pt-2 grid grid-cols-2 gap-3">
                  <button 
                    ...
                    className="flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white py-2 rounded-lg text-sm font-medium transition-colors cursor-pointer"
                  >
                    <Download size={16} /> PDF
                  </button>
                  <button 
                    onClick={handlePrint} // Trigger Print Dialog
                    className="flex items-center justify-center gap-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 py-2 rounded-lg text-sm font-medium transition-colors cursor-pointer"
                  >
              ...
              <div id="printable-report" className="bg-white rounded-2xl shadow-sm border border-slate-200 min-h-[500px] flex flex-col">
                ...
                <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div>
                    ...
                    {/* Add Patient Name here for print only if sidebar is hidden */}
                ...
                <div className="p-6">
                   ...
                   {/* Other states (Uploaded/Pending/Comments) ... same as before */}
                   {reportData.entryType === 'Upload' && reportData.reportDocument && (
                       <div className="text-center p-12 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50 no-print">
                           {/* ... upload UI (hidden on print usually) ... */}
                           <p className="text-slate-600 font-medium">Report Document Available online.</p>
                       </div>
                   )}
