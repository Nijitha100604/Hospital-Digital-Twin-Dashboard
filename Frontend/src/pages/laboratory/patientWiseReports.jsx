        ...
        {loading && !reportData ? (
          ...
          <div className="bg-white border border-dashed border-slate-300 rounded-2xl p-12 text-center flex flex-col items-center justify-center min-h-[400px]">
            ...
            <p className="text-slate-500 text-sm mt-1 max-w-xs">
              Enter a Patient ID or Name in the search bar above to view their
              report.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in duration-500">
                ...
                <div className="bg-slate-900 p-6 text-white flex items-center gap-4">
                  <div className="w-14 h-14 bg-white/10 rounded-full flex items-center justify-center text-xl font-bold border border-white/20">
                    {reportData.patientName
                      ? reportData.patientName.substring(0, 2).toUpperCase()
                      : "PT"}
                  </div>
                  <div>
            ...
            <div className="lg:col-span-8">
              <div
                id="printable-report"
                className="bg-white rounded-2xl shadow-sm border border-slate-200 min-h-[500px] flex flex-col"
              >
                {/* Report Header */}
                            ...
                            {reportData.testResults.map((row, idx) => (
                              <tr key={idx} className="hover:bg-slate-50/50">
                                ...
                                <td className="px-6 py-3 text-right">
                                  <span
                                    className={`inline-block px-2.5 py-0.5 rounded text-[10px] font-bold uppercase border ${getStatusStyle(row.status)}`}
                                  >
                                    {row.status}
                                  </span>
                                </td>
